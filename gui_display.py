...
class DouyinLiveGUI(QMainWindow):
    ...
    def connect_room(self):
        ...
        try:
            ...
            if room_status == 0:
                ...
                # 添加直播链接到消息中
                self.chat_area.append(f"{room_id} - {nickname}正在直播！")
                self.chat_area.append(f"<a href='{live_url}' style='color: blue;'>点击这里查看直播间</a>")
            else:
                # 其他状态下也尝试连接，可能正在准备中
                self.chat_area.append(f"{room_id} - {nickname}当前状态: {room_status}，正在尝试连接...")
    ...
    def display_room_status(self, nickname, user_id, status):
        """显示房间状态信息"""
        status_msg = f"【{nickname}】[{user_id}]直播间：{status}"
        self.chat_area.append(status_msg)  # 也在聊天区域显示
        self.stats_area.append(status_msg)

        # 如果状态为"已结束"，则在聊天区域显示提示
        if status == "已结束":
            self.chat_area.append("直播已结束，可尝试重新连接或切换其他直播间")
